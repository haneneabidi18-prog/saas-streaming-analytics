...
def generate_pdf(total_views, total_watch_time, total_revenue, insights):
    ...
    elements.append(Paragraph(
        "Based on the analysis, prioritize top-performing content, strengthen high-revenue markets, "
        "optimize device experience, and investigate quality issues impacting playback.",
        normal_style
    ))
...
def generate_insights(filtered_df, total_views, total_revenue, total_watch_time):
    ...
    if "buffering_ratio" in filtered_df.columns:
        ...
        if avg_buffering > 0.2:
            insights.append(
                f"Alert: High buffering detected. Average buffering ratio is {avg_buffering:.2f}."
            )
    ...
    if "startup_time_ms" in filtered_df.columns:
        ...
        if avg_startup > 4000:
            insights.append(
                f"Alert: High startup time detected. Average startup time is {avg_startup:.0f} ms."
            )
    ...
    if "error_code" in filtered_df.columns:
        ...
        if error_rate > 20:
            insights.append(
                f"Alert: High error rate detected ({error_rate:.1f}%). Investigate delivery pipeline."
            )
